...
others, keep up and organise their contact list. With the cloud import feature, a user will be able to get HitMeUp
running in a matter of minutes. Being able to group your contacts and view each group at a glance offers convenience like never before. Social
integration with Instagram and Twitter also allows users to view their contacts' social media profiles in HitMeUp itself without
...
of the underlying mechanism of certain features that are implemented as well as the different aspects of implementation
...
public CommandResult execute() throws CommandException {
    ...
    for (Index targetIndex : targetIndices) {
        ...
        // Appends all names and email addresses of the persons to email
    ...
    // Replaces all white spaces in the addresses String with commas
...
public void handleEmail(String allEmailAddresses) {
    ...
    if (Desktop.isDesktopSupported()) {
        ...
        try {
            // Desktop mail method will launch user default mail client
...
[NOTE]
To construct a valid `URI` instance, the String `EMAIL_URI_PREFIX` which specifies the scheme to launch user's default mail client must be
...
**Alternative 1 (current choice):** Use user's default mail client in their desktop +
**Pros:** The default mail client tends to load faster than the Javafx `WebView` used by the `BrowserPanel` in the `UI`. +
**Cons:** Some users may have set their default mail client as other applications such as web browser where they have to make changes before it can be used. +
...
**Pros:** It allows user to format their messages using text editor in default mail client. +
...
This file will be saved in the application data folder where user can import it into other platforms such as Google Contacts.
...
method write to write and saved all information in the `File` named as "contacts.vcf". An example of how the write method is executed is shown below:
...
// For each Vcard created earlier
    try {
        // cardDetails are first encoded before FileOutputStream writes the encoded string into the file.
...
---------------
[NOTE]
`cardDetails` are first encoded into a sequence of bytes and stored in a byte array.
`FileOutputStream` calls the write method in order to write the encoded string into the file.
