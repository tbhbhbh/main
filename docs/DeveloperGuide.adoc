...
This feature is implemented to provide users the utility to email their contacts using the user's default mail client
to launch an email application which will contain the email addresses of contacts specified by user. +
User can edit the subject field and compose the email using their own default email application.

==== Email feature implementation
When user enter the command to email one or more person in their address book. `EmailCommandParser`
will parse the arguments which are indices and store these indices provided by the user's input
in an array called `targetIndices` and return a `EmailCommand`. +
`EmailComand` will ensure that `Index` specified is valid and construct a String `allEmaillAddress`
containing the email addresses separated by a comma. For example: +
...
public CommandResult execute() throws CommandException {
        ...
        for (Index targetIndex : targetIndexes) {
            if (targetIndex.getZeroBased() >= lastShownList.size()) {
                //Throw exception due to invalid index
            }
...
-------------
Next, we are using `java.awt.Desktop` package to launch the default mail client registered on
the native desktop to handle a `URI`. The `URI mailTo` instance is created by using a String prefix
which specifies the operation to launch user's default mail client and is followed by the String `allEmailAddresses`. Lastly, user's desktop will launch
the mail client. Refer to the code example below: +
...
URI mailTo = new URI(EMAIL_URI_PREFIX + allEmailAddresses);
    //Checks if user Desktop applications are supported
...
**Pros:** Default mail client tend to load faster than the default browser `javafx.scene.web.WebView` used by `UI` `BrowserPanel`. +
**Cons:** Does not support some operating systems such as Linux. +
**Alternative 2:** Use web browser to access user email online. +
**Pros:** Able to support more users to use this email feature. +
**Cons:** Not all users use the same email services such as Gmail or Outlook. Hence,
it will be difficult to authorize the web pages or API access for them and it is often dependent
