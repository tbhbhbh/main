...
**Aspect:** Type of browser to support the email command +
**Alternative 1 (current choice):** Using java `Desktop` class +
**Pros:** It can launch the default email application that loads quickly. +
**Cons:** `Desktop` API may not be available in some platforms such as Linux +
**Alternative 2:** Using javafx `WebView` class +
**Pros:** Able to run HTML5 applications on desktop systems that lack a compatible web browser +
**Cons:** Web pages load slower and google mail will load in basic HTML view.
